...
def test_nstage_local_cov_smoke():
    ...
    ellcloud = ellipse_cloud(X, method="nstage_local_cov", k=k, n_stages=n_stages)

    # Check that the output is an EllipseCloud instance
    assert isinstance(ellcloud, EllipseCloud)

    # Check that the number of ellipses is correct (can be smaller due to unique subsets)
