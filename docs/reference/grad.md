...
Gradients are derived via the **envelope theorem**: the derivative through the optimal tangent point $\mathbf{x}^*$ vanishes, so only the explicit dependence of the pencil on the input coefficients survives.  This makes the gradient cheap — no second solver pass is required.
